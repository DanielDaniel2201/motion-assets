...
import { ChevronLeftIcon, CloseIcon, ExportIcon, ReplayIcon } from "./components/icons";
import { ParameterSlider } from "./components/ParameterSlider";
import { PreviewCanvas } from "./components/PreviewCanvas";
import { createMovDownload, startCardStackExport, triggerMovDownload, type ExportProgress, type ExportTask } from "./export/client";
...
export function App() {
  ...
  useEffect(() => {
    if (
      activeAsset !== cardStackDefinition.id
  ...
  const exportMov = async () => {
    ...
    setIsExporting(true);
    setExportProgress({ progress: 0, frame: 0, totalFrames: Math.ceil(duration * 30) });
    const task = startCardStackExport(
      {
        id: crypto.randomUUID(),
        type: "export",
    ...
    try {
      ...
      const download = createMovDownload(blob);
  ...
  const cancelExport = () => exportTaskRef.current?.cancel();
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        >
          ...
          <PreviewCanvas images={sources} parameters={parameters} replayToken={replayToken} width={outputFormat.width} height={outputFormat.height} />
